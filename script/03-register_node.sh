...
installed=$(psql -qAt -h "$PGHOST" -U "$PG_REP_USER" "$REPMGR_DB" -c "SELECT 1 FROM pg_tables WHERE tablename='nodes'")
...
if [ -n "$WITNESS" ]; then
	...
	repmgr -h "$PRIMARY_NODE" -U "$PG_REP_USER" -d "$REPMGR_DB" witness register
	return
fi
...
if [ "${is_reg}" != "1" ] && [ ${my_node} -gt 1 ]; then
    ...
    rm -Rf "$PGDATA"/*
    repmgr -h "$PRIMARY_NODE" -U "$PG_REP_USER" -d "$PG_REP_DB" standby clone --fast-checkpoint
    cp /etc/postgresql/postgresql.conf /var/lib/postgresql/data/
    pg_ctl -D "$PGDATA" start &
    sleep 1
    repmgr -h "$PRIMARY_NODE" -U "$PG_REP_USER" -d "$PG_REP_DB" standby register
fi