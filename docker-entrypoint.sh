...
if [ "$1" = 'postgres' ]; then
  #create sys_group user and sys_group if not exist
	...
	if [ ! -s "$PGDATA/PG_VERSION" ]; then
		...
		if ! getent passwd ${SYS_USER} &> /dev/null && [ -e /usr/lib/libnss_wrapper.so ]; then
			export LD_PRELOAD='/usr/lib/libnss_wrapper.so'
			export NSS_WRAPPER_PASSWD="$(mktemp)"
			export NSS_WRAPPER_GROUP="$(mktemp)"
			echo "postgres:x:${SYS_USER}:${SYS_GROUP}:PostgreSQL:$PGDATA:/bin/false" > "$NSS_WRAPPER_PASSWD"
			echo "postgres:x:${SYS_GROUP}:" > "$NSS_WRAPPER_GROUP"
		fi
    ...
    if [ -z "$PGHOST" ]; then
      ...
      is_active=$(psql -qAt -h "$PGHOST" -U "$PG_REP_USER" -d "$PG_REP_DB" -p "$PRIMARY_NODE_PORT" -c "SELECT active FROM repmgr.nodes WHERE node_id=${my_node}")
